# FORM-04 - Submit with an empty first name

Verify that submitting the sign-up form with no first-name value blocks submission and reports the required-field error.

Submission is blocked, the required-field error is shown, and no account-creation request is sent.
...
compliant generated password in both password fields, and select consent. Leave the first-name field empty.

## When

Submit the completed form with the first-name field empty while monitoring for an account-creation request.

## Then

Submission is blocked. The first-name field shows the localized required error: `The field is required` in `en-CA` or
...
| 3 | Leave the first-name field empty and submit the form. | Submission is blocked and the localized required error is shown for first name. |
