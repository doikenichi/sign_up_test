# FORM-27 - Reject a control character in the first name

Verify that a first name containing a control character is rejected before account creation.
...
Complete every required field with isolated, non-identifying valid test data, select Canada, and select consent. Use `Anne\u0007` as the first-name value, where `\u0007` is the BEL control character.

## When

Submit the completed form with the control character in the first-name field while monitoring for an account-creation request.

## Then

The first-name field shows the localized invalid-name error: `Invalid name` in `en-CA` or `Nom invalide` in `fr-CA`. No account-creation request is sent.
...
| 2 | Fill all required fields with valid isolated test data, select Canada and consent. | All fields except the intentionally invalid first name accept their values. |
