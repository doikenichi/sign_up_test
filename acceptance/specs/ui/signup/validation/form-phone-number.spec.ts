...
test.describe("Phone-number validation", () => {
    test("FORM-11: handles an empty phone number", async ({
    ...
    for (const invalidCase of invalidNumbers) {
        test(`FORM-12: handles phone number with ${invalidCase.description}`, async ({
                                                                                         interceptNetworkCall,
                                                                                         signUpPage,
                                                                                     }) => {
            // Arrange
    ...
    test("FORM-13: accepts a valid Brazilian phone number", async ({
                                                                       signUpPage,
                                                                   }) => {
        // Arrange
        await completeSignUpForm(signUpPage, {
    ...
    test("FORM-14: accepts a valid Canadian phone number", async ({
                                                                      signUpPage,
                                                                  }) => {
        // Arrange
        await completeSignUpForm(signUpPage, {
