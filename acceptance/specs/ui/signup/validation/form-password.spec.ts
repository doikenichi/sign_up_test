...
test.describe("Password validation", () => {
    test("FORM-18: handles an empty password", async ({
    ...
    test("FORM-19: handles a password below the 12-character minimum", async ({
                                                                                  content,
                                                                                  interceptNetworkCall,
                                                                                  signUpPage,
                                                                              }) => {
    ...
    test("FORM-20: handles a password above the 32-character maximum", async ({
                                                                                  interceptNetworkCall,
                                                                                  signUpPage,
                                                                              }) => {
        // Arrange
    ...
    for (const invalidCase of missingCharacterClasses) {
        test(`FORM-21: handles a password missing required ${invalidCase.description}`, async ({
                                                                                                   content,
                                                                                                   interceptNetworkCall,
                                                                                                   signUpPage,
                                                                                               }) => {
    ...
    for (const validCase of compliantPasswords) {
        test(`FORM-22: accepts a compliant password at the ${validCase.description}`, async ({
                                                                                                 signUpPage,
                                                                                             }) => {
            // Arrange
            await completeSignUpForm(signUpPage, {
